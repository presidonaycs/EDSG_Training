      ...
      <table className="table-view m-t-10">
        ...
        <tbody className="table-body">
          {!(isLoading || errorMsg) && details && details.map((item) => (
            <ListItem key={item.id} details={item} editTrainingRequest={editRequest} />
          ))}
        </tbody>
      </table>
...
const ListItem = ({ details = {}, editTrainingRequest }) => {
  ...
  const deleteItem = async (e) => {
    e && e.preventDefault();
    e && e.stopPropagation();
    const confirmAction = confirm(`Click OK to confirm the deletion of "${details.title}"`);
    ...
    if (res && res.status === 200) {
      ...
      notification({
        title: 'Network Error',
        message: 'Something went wrong while deleting a Training. Please, try again later.',
        type: 'danger'
      });
    }
    setIsDeleting(false);
