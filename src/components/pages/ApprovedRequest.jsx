...
const ApprovedRequest = ({ history }) => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    document.title = "EDSG - Approved Training Requests";

    const getList = async () => {
      setIsLoading(true);
      setErrorMsg('');
      const res = await get({ endpoint: '/request/approvedrequest' });
  ...
  return (
    <div className="w-100">
      ...
      <div className="w-100 m-t-20">
        <div className="w-100">
          <h3>Approved Training Requests</h3>
        </div>
