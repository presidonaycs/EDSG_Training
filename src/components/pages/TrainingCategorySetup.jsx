...
import TrainingCategoryModal from "../layouts/TrainingCategoryModal";

import notification from '../../utility/notification';
import { get, del } from '../../api-services/fetch';

const TrainingCategorySetup = ({ history }) => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    document.title = 'EDSG - Training Category Setup';
  ...
  const getRoleList = async ({ query = null }) => {
    ...
    setIsLoading(true);
    setErrorMsg('');
    const res = await get({ endpoint: 'category-setup', pQuery: { query }});
  ...
  const deleteItem = async (id, name) => {
    ...
    setIsDeleting(true);
    const res = await del({ endpoint: 'category-setup', param: id });
    ...
    if (res && res.status === 200) {
      ...
      if (data.code === 1) {
        notification({
          title: 'Successful Category Deletion',
      ...
      notification({
        title: 'Network Error',
        message: 'Something went wrong while deleting a Category. Please, try again later.',
        type: 'danger'
      });
    }
    setIsDeleting(false);
  ...
  return (
    <div className="w-100">
      <div className="w-100 flex space-between flex-v-center m-b-20">
        <h3>Training Category Setup</h3>
        {/* <LocationDetails /> */}
      </div>
      <div className="table-view m-t-10">
        <div className="table-header">
          <div className="table-row">
            <div className="th"><p>Training Category</p></div>
        ...
        <div className="table-body">
          {!(isLoading || errorMsg) && details && details.map((item) => (
            <div className="table-row" key={item.id}>
              <div className="td" style={{ maxWidth: '350px' }}>{item.category}</div>
              ...
              <div className="td actions">
                ...
                <ReactTooltip id="del-btn" type="error" effect="solid">
                  <span>Delete Category Details</span>
                </ReactTooltip>
              </div>
            </div>
          ))}
      ...
      {showFormModal && (
        <TrainingCategoryModal details={selectedRecord} updateList={updateList} closeModal={closeModal} />
      )}
    </div>
  );
};

export default TrainingCategorySetup;
