...
import TrainingApproverModal from "../layouts/TrainingApproverModal";

import notification from '../../utility/notification';
import { get, del } from '../../api-services/fetch';

const TrainingApproverSetup = ({ history }) => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    document.title = 'EDSG - Training Approver Setup';
  ...
  const getRoleList = async ({ query = null }) => {
    ...
    setIsLoading(true);
    setErrorMsg('');
    const res = await get({ endpoint: 'approval-setup', pQuery: { query }});
    ...
    if (res && res.status === 200) {
      ...
      if (data.code === 1) {
        ...
        if (data.data.length < 1) {
          if (query) {
            setErrorMsg('Approver not found');
          } else {
            setErrorMsg('No Approver yet');
          }
        }
      } else {
  ...
  const deleteItem = async (id, name) => {
    ...
    setIsDeleting(true);
    const res = await del({ endpoint: 'approval-setup', param: id });
    ...
    if (res && res.status === 200) {
      ...
      if (data.code === 1) {
        notification({
          title: 'Successful Approver Deletion',
      ...
      notification({
        title: 'Network Error',
        message: 'Something went wrong while deleting a Approver. Please, try again later.',
        type: 'danger'
      });
    }
    setIsDeleting(false);
  ...
  return (
    <div className="w-100">
      <div className="w-100 flex space-between flex-v-center m-b-20">
        <h3>Training Approver Setup</h3>
        {/* <LocationDetails /> */}
      </div>
      <div className="table-view m-t-10">
        <div className="table-header">
          <div className="table-row">
            <div className="th"><p>List</p></div>
            <div className="th"><p>Approval Sequence</p></div>
            ...
            <div className="table-row" key={item.id}>
              ...
              <div className="td actions">
                ...
                <ReactTooltip id="view-btn" type="warning" effect="solid">
                  <span>Edit Aprover Details</span>
                </ReactTooltip>
                <button
                  data-tip
                  data-for="del-btn"
                  ...
                  onClick={() => deleteItem(item.id, item.role)}
                >
                  <GiTrashCan />
                </button>
                <ReactTooltip id="del-btn" type="error" effect="solid">
                  <span>Delete Aprover Details</span>
                </ReactTooltip>
              </div>
            </div>
          ))}
      ...
      {showFormModal && (
        <TrainingApproverModal details={selectedRecord} updateList={updateList} closeModal={closeModal} />
      )}
    </div>
  );
};

export default TrainingApproverSetup;
