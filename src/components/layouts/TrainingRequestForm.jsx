...
const TrainingRequestForm = ({ details, closeForm }) => {
  ...
  const [localTransportationExpense, setLocalTransportationExpense] =
    useState(null);
  const [journalPublicationExpense, setJournalPublicationExpense] =
    useState(null);
  ...
  useEffect(() => {
    ...
    if (details && details.id) {
      ...
      setStartDate(
        (details.startDate && formatDateforPicker(details.startDate)) || ""
      );
      setEndDate(
        (details.endDate && formatDateforPicker(details.endDate)) || ""
      );
    ...
    res?.data &&
      setApproversOptions(
        res?.data
          ...
          .filter((item) => {
            return !item.label.includes(Cookies.get("fullname"));
          })
      );
  };
  ...
  const validateForm = () => {
    ...
    if (!impactOnindividual)
      return "Please, enter expected impact on individual for the request";
    if (!impactOnOrganisation)
      return "Please, enter expected impact on organisation name for the request";
    if (!startDate) return "Please, enter a start date for the request";
    if (getDaysFromDate(startDate) < 0)
      return "Start date must not be behind today";
    if (!endDate) return "Please, enter a end date for the request";
    if (approverId.length < 1)
      return "Please select at least one approver fo the request.";
  ...
  const submitForm = async (type) => {
    ...
    if (type === "save") {
      ...
      res =
        formType === "create"
          ? await post({
              endpoint: "/Request/saveandsubmitrequest",
              body: payload,
            })
  ...
  const handleFilesUpload = async () => {
    ...
    uploadField.onchange = async () => {
      ...
      //     message:
      //       "You can only upload one document for your training request.",
      //     type: "danger",
      //   });
      //   return;
      // }
      ...
      const newDocumentsForUpload = [...documentsForUpload];
      ...
      if (setSize < b64FileSet.size) {
        ...
        body.append("files", uploadField.files[0]);
        ...
        if (response && response.status === 200) {
          if (response.data.code === 1) {
            newDocumentsForUpload.push({
              id: 0,
              docPath: response.data.doclink,
              docName: uploadField.files[0]?.name,
            });
            notification({
      ...
      setDocumentsForUpload(newDocumentsForUpload);
    };
  };
  ...
  const handleFileDelete = async (index) => {
    ...
    const newDocumentsForUpload = documentsForUpload.filter(
      (val, i) => i !== index
    );
